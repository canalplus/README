 ...
 */
export default function generatePageHtml({
  contentHtml,
  cssUrls,
  faviconUrl,
  navBarHtml,
...
function constructHamburgerBarHtml(pageListHtml: string): string {
  return (
 ...
 * @returns {string}
 */
function constructStylesHtml(cssUrls: string[]): string {
  return cssUrls
 ...
 * @returns {string}
 */
function constructScriptsHtml(scriptUrls: string[]): string {
  return scriptUrls
    .map(
      (scriptUrl) =>
